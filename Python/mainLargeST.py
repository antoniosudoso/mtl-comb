...
from util import standardize_series, get_data_LargeST
from networks import build_baseline, build_network
import argparse

##################### Set this path to the full path of the Metalearning directory #####################
...
if __name__ == '__main__':
    ...
    f_model, w_model, reg_model, cls_model = build_network(MAX_LEN, F_METHODS, F_HORIZON, LAMBDA, 1.0)
    f_model.summary()
    ...
    history = f_model.fit([ts_train, ts_mat_forecast, ts_div_label], ts_test, epochs=1000, shuffle=True, batch_size=32, validation_split=0.2, callbacks=[es_callback])
    
    f_model.save('./saved_model_LargeST/' + SERIES_TYPE + '/main/f_model_' + str(LAMBDA))
    w_model.save('./saved_model_LargeST/' + SERIES_TYPE + '/main/w_model_' + str(LAMBDA))
