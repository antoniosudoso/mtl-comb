...
f_series <- readRDS(paste0(paste0("test/forecast_test_", SERIES_TYPE), ".rds"))
df_series <- df_series[1:length(f_series)]
df <- create_test_series_data(df_series, f_series)
# main .rds for testing neural network (needed for computing final tables)
saveRDS(df, file=paste0(paste0("test/series_test_", SERIES_TYPE), ".rds"))
