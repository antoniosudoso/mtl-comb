...
df_series <- readRDS(file=paste0(paste0(paste0("./LargeST/LargeST_val_"), SERIES_TYPE), ".rds"))
#df_series <- df_series[1:16000]
print(length(df_series))


# This .rds has been generated by running 'mainForecast.R'
f_series <- readRDS(paste0(paste0("./LargeST/data/forecast_val_", SERIES_TYPE), ".rds"))
df <- create_df(f_series, df_series)
df_errors <- compute_errors(df)

# Save the matrix of forecasting errors for each series
saveRDS(df_errors, file=paste0(paste0("./LargeST/data/raw_labels_", SERIES_TYPE), ".rds"))